...
local environment = assert(getgenv, "getgenv is required")()
...
function handlers.listInstances(params)
    ...
    for _, child in ipairs(instance:GetChildren()) do
        if className == "" or child.ClassName == className then
            local described = describeInstance(child)
            if query == ""
...
function handlers.readSource(params)
    ...
    local source = sourceCache[instance]
    if not source then
        source = decompile(instance)
        sourceCache[instance] = source
    end
...
function handlers.eval(params)
    local chunk, compileError = loadstring(params.code, params.chunkName or "live-mcp")
    if not chunk then
        error(compileError)
    end
